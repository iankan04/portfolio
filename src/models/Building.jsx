...
export default function Building({ isRotating, setIsRotating, setCurrentStage, ...props}) {
  ...
  const rotationSpeed = 0.02 * Math.PI;
  const rotationVelocity = useRef(0);
  const dampingFactor = useRef(1);
  ...
  useEffect(() => {
    if (actions['Take 001']) {
      setIsActionReady(true); // Mark action as ready
  ...
  const handlePointerMove = (e) => {
    ...
    if (isRotating) {
      ...
      lastX.current = clientX;
      rotationVelocity.current = delta * 0.01 * Math.PI;
    }
  }
  ...
  const waitForPause = (callback) => {
    const interval = setInterval(() => {
      if (action.paused) {
        clearInterval(interval); // Stop checking once the animation is paused
        callback(); // Execute the callback
      }
    }, 50); // Check every 50ms
  };

  const handleButtonClicked = (targetTime, index) => {
    setCurrentStage(null);
    const currentTime = action.time;

    if (!action.isRunning() && !action.paused && index !== 0) {
      startAnimation();
    }
  ...
  const handleProfileButtonClicked = () => handleButtonClicked(0, 0);

  const handleAboutButtonClicked = () => handleButtonClicked(1.3, 1);

  const handleProjectsButtonClicked = () => handleButtonClicked(2.4, 2);

  const handleContactButtonClicked = () => handleButtonClicked(4.2, 3)
 
  useFrame(() => {
    if (buildingRef.current) {
      buildingRef.current.rotation.y += rotationVelocity.current;

      if (!isKeyPressed.current && !isRotating) {
        rotationVelocity.current *= dampingFactor.current;
  ...
  return (
    <a.group ref={buildingRef} {...props} dispose={null}>
      <group name="Sketchfab_Scene">
        <group name="Sketchfab_model" rotation={[-Math.PI / 2, 0, 0]} scale={0.013} position={[0.7, 0, 0.1]}>
          {isActionReady && <Html 
            position={[100, 200, -75]} 
          ...
          >
            <button
              ...
              className='bg-white/20 border-100 border-white w-400 h-400 text-white text-[500px] font-bold rounded-full hover:border-blue-500 cursor-pointer'
          ...
          >
            <button
              onClick={handleAboutButtonClicked}
              className='bg-white/20 border-100 border-white w-400 h-400 text-white text-[500px] font-bold rounded-full hover:border-blue-500 cursor-pointer'
              disabled={isAnimationRunning}
            >
              2
          ...
          >
            <button
              onClick={handleProjectsButtonClicked}
              className='bg-white/20 border-100 border-white w-400 h-400 text-white text-[500px] font-bold rounded-full hover:border-blue-500 cursor-pointer'
              disabled={isAnimationRunning}
            >
              3
